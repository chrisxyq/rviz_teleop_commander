...
class MoveItDemo:
    def __init__(self):
        ...
        obstacle1_pose.header.frame_id = arm.get_planning_frame()
        ...
        arm_6_link = arm.get_end_effector_link()        
        ...
        arm_reference_frame = arm.get_planning_frame()        
        #控制机械臂运动到初始位姿
        ...
        pose3 = PoseStamped()
        pose4 = PoseStamped()
        pose1.header.frame_id = arm_reference_frame
        pose2.header.frame_id = arm_reference_frame
        pose3.header.frame_id = arm_reference_frame
        pose4.header.frame_id = arm_reference_frame
        ...
        arm.set_pose_target(pose1, arm_6_link )        
        #规划并执行运动轨迹
        arm.go()  
        #traj =arm.plan()
        #arm.execute(traj)          
        ...
        start_pose = arm.get_current_pose(arm_6_link).pose
        ...
        arm.set_pose_target(pose2, arm_6_link )        
        #规划并执行运动轨迹
        arm.go()
        #traj =arm.plan()
        #arm.execute(traj)  
        ...
        for i in range(1,360/angle_resolution):
          ...
          waypoints2.append(deepcopy(wpose))    
         #使用compute_cartesian_path()函数计算笛卡尔空间轨迹，输入参数包括
        (cartesian_plan2, fraction) = arm.compute_cartesian_path (
                                        waypoints2,   # waypoint poses
                                        0.02,        # eef_step 插值分辨率1cm
                                        0.0,         # jump_threshold 跳跃阈值
                                        True)        # avoid_collisions 是否需要避障                            
        #traj =cartesian_plan2
        arm.execute(cartesian_plan2)
        rospy.sleep(1)


        arm.set_pose_target(pose3, arm_6_link )        
        #规划并执行运动轨迹
        arm.go()
        #traj =arm.plan()
        #arm.execute(traj)  
        ...
        for i in range(1,360/angle_resolution):
          ...
          waypoints3.append(deepcopy(wpose))    
         #使用compute_cartesian_path()函数计算笛卡尔空间轨迹，输入参数包括
        (cartesian_plan3, fraction) = arm.compute_cartesian_path (
                                        waypoints3,   # waypoint poses
                                        0.02,        # eef_step 插值分辨率1cm
                                        0.0,         # jump_threshold 跳跃阈值
                                        True)        # avoid_collisions 是否需要避障                            
        #traj =cartesian_plan3
        arm.execute(cartesian_plan3)
        rospy.sleep(1)


        arm.set_pose_target(pose4, arm_6_link )        
        #规划并执行运动轨迹
        arm.go()
        #traj =arm.plan()
        #arm.execute(traj)  
        ...
        for i in range(1,360/angle_resolution):
          ...
          waypoints4.append(deepcopy(wpose))    
         #使用compute_cartesian_path()函数计算笛卡尔空间轨迹，输入参数包括
        (cartesian_plan4, fraction) = arm.compute_cartesian_path (
                                        waypoints4,   # waypoint poses
                                        0.02,        # eef_step 插值分辨率1cm
                                        0.0,         # jump_threshold 跳跃阈值
                                        True)        # avoid_collisions 是否需要避障                            
        #traj =cartesian_plan4
        arm.execute(cartesian_plan4)
        rospy.sleep(1)
