...
class MainCode(QMainWindow, paint.Ui_MainWindow):
    def __init__(self):
        ...
        self.setupUi(self)
        self.setWindowIcon(QIcon('/home/cp3/Downloads/joey.jpeg'))
    ...
    def on_open_file(self):
        ...
        #从全路径获取文件名
        index = os.path.basename(dir).rfind('.')
        name = os.path.basename(dir)[:index]
        global name
        print name
        global txtpath
        txtpath=name+'.txt'
    ...
    def on_pause_paint(self):
        ...
        global qmut
        qmut.unlock()
        QMessageBox(QMessageBox.Information, "提示", "暂停成功!").exec_()
        self.statusbar.showMessage("成功暂停，已画"+str((i + 1) / 2)+"段，共"+str(len(pts) / 2)+"段画图轨迹")


    def on_continue_paint(self):
        '''继续画图按钮'''
        global i, pts
        QMessageBox(QMessageBox.Information, "提示", "画图继续!").exec_()
        self.statusbar.showMessage("画图继续，从" + str((i + 1) / 2) + "段继续，共" + str(len(pts) / 2) + "段画图轨迹")
    ...
    def on_default_paint(self):
        '''
        默认不标定坐标系开始画图
        线程启动后，会调用其实现的run方法，该方法就是线程的执行函数，当run()退出之后线程基本就结束了。
        '''
        self.default_paint.setEnabled(False)
        self.thread = RunThread()
        self.thread.start()
        self.default_paint.setEnabled(True)
    ...
    def run(self):
        ...
        if len(name)!=0:
            ...
            for i in range(len(pts)):
                if i % 2 == 0:
                    QMainWindow.statusbar.showMessage("i="+str(i))
                    '''抬笔点:绝对赋值'''
                ...
                if i % 2 != 0:
                    ...
                    target.position.z = pose.position.z - h1
                    self.cartesian(target)
            QMessageBox(QMessageBox.Information, "提示", "画图结束!").exec_()
            moveit_commander.roscpp_shutdown()
        else:
            QMessageBox(QMessageBox.Warning, "警告", "请先选择画图jpg!").exec_()
            QMainWindow.statusbar.showMessage("请先选择画图jpg!")
        qmut.unlock()# 解锁
        # 完毕后发出信号
        #self.trigger.emit()
